...
#include "ewa_base/util/dcontext.h"
#include "ewa_base/basic/scanner_helper.h"

EW_ENTER

void DCtxStateMaterial::init(CallableSymbol* p,CallableSymbol* b)
{
	arr.clear();
	stk.clear();
...
void DCtxStateMaterial::push(CallableSymbol* p)
{
...
void DContext::RenderObject(CallableSymbol* p)
{
	if (!p) return;
	p->DoRender(*this);
}
